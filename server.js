...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        connection.query(`INSERT INTO details(UserName,UserEmailId,Hashedpassword) VALUES('${username}','${email}','${hashedpassword}')`, (err, result) => {
            if (err) {
                if('ER_DUP_ENTRY')
                res.status(400).send("This email is already have been register",err);
            }
        });
    ...
    const RegisterMail = {
        ...
        text: `${username} You successfully Register into my Assignment project demo. Thankyou.`
    ...
    try {
        ...
        console.log("Error occured while send mail", err);
    }
    } catch (err) {
    ...
    try {
        ...
        connection.query(query, [email], async (err, result) => {
            ...
            if (isMatch) {
                ...
                const token=jwt.sign(userpayload,jwt_token_secret,{expiresIn:'12day'});
